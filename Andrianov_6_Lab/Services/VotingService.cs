...
public class VotingService
{
    ...
    {
        var settingsQuery = @"
            SELECT c.session_id, s.multi_select, s.max_choices, s.allow_vote_change_until_close
            FROM voting.candidates c
            JOIN voting.voting_settings s ON s.session_id = c.session_id
            WHERE c.id = @candidateId";

        var settingsRows = await _executor.ExecuteQueryAsync(settingsQuery, new NpgsqlParameter("candidateId", candidateId));
        if (!settingsRows.Any())
        {
            throw new InvalidOperationException("Candidate not found for voting.");
        }

        var settingsRow = settingsRows.First();
        var sessionId = Guid.Parse(settingsRow["session_id"]?.ToString() ?? Guid.Empty.ToString());
        var multiSelect = bool.TryParse(settingsRow["multi_select"]?.ToString(), out var multi) && multi;
        var maxChoices = int.TryParse(settingsRow["max_choices"]?.ToString(), out var max) ? max : 1;
        var allowChange = bool.TryParse(settingsRow["allow_vote_change_until_close"]?.ToString(), out var allow) && allow;
        ...
        if (userId != Guid.Empty)
        {
            const string countSql = @"
                ...
                FROM voting.votes v
                JOIN voting.candidates c ON v.candidate_id = c.id
                WHERE v.user_id = @userId AND c.session_id = @sessionId AND v.is_valid = TRUE";

            var existingCountRows = await _executor.ExecuteQueryAsync(countSql,
                new NpgsqlParameter("userId", userId),
                new NpgsqlParameter("sessionId", sessionId));

            var existingVotes = existingCountRows.Any()
                ? Convert.ToInt32(existingCountRows.First()["vote_count"] ?? 0)
                : 0;

            if (existingVotes >= maxChoices)
            {
                if (allowChange)
                {
                    const string deleteSql = @"
                        DELETE FROM voting.votes v
                        USING voting.candidates c
                        WHERE v.candidate_id = c.id AND v.user_id = @userId AND c.session_id = @sessionId";

                    await _executor.ExecuteNonQueryAsync(deleteSql,
                        new NpgsqlParameter("userId", userId),
                        new NpgsqlParameter("sessionId", sessionId));
                }
                else
                {
                    throw new InvalidOperationException(multiSelect
                        ? $"Достигнут лимит голосов: максимум {maxChoices} в этой сессии."
                        : "Вы уже голосовали в этой сессии.");
                }
            }
        }
